  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[750px] p-0 overflow-hidden gap-0 bg-white shadow-2xl rounded-2xl border-0">
        ...
        <div className="bg-gray-50/80 p-5 sm:p-8 border-b border-gray-100 flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6 relative">
           {/* Hueco para futura imagen real */}
           <div className="w-24 h-24 sm:w-32 sm:h-32 bg-white rounded-2xl border border-gray-200 flex items-center justify-center shadow-sm shrink-0">
             <span className="text-gray-300 font-semibold text-xs">{t('common.no_img')}</span>
           </div>
           ...
           <div className="flex-1 space-y-2 sm:space-y-3 text-center sm:text-left">
             <div className="flex items-center justify-center sm:justify-start gap-2 sm:gap-3 mb-1">
               <span className="text-[9px] sm:text-[10px] uppercase font-bold tracking-wider bg-blue-100/50 text-blue-700 px-2 sm:px-2.5 py-1 rounded-md border border-blue-100">
             ...
             <div className="flex items-end justify-center sm:justify-start gap-2 pt-1">
               ...
               <span className="text-xs sm:text-sm font-medium text-gray-500 mb-1 sm:mb-1.5">/ud (Sin IVA)</span>
             </div>
           </div>
        </div>

        {/* Cuerpo Inferior: Descripción, Specs y Controles */}
        <div className="p-5 sm:p-8 grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10 bg-white">
